...
|---|---|---|---|---|---|
| GitHub | 代码仓、评审、Actions、MCP | Agentic Workflows/CI Doctor、Code Scanning Agentic Autofix、GitHub MCP | L3 | Workflows/Autofix 公开预览；Code Review/MCP 另有 GA 能力 | 用 Actions、Safe Output、Scanner 与 Ruleset 分离推理、写入和接受 |
...
| GitLab | 一体化 DevSecOps | Duo Agent Platform、Review/Fix CI/SAST Flow、Orbit | L3 | Platform/Fix Flow GA；其他 Agent、Governance、Orbit 子能力混合 | 以全生命周期上下文和治理做差异化；完整 Pipeline 复验仍需逐 Flow 核对 |
| Harness | 跨软件交付 Agent 控制面 | DevOps Agent、Worker Agents、Knowledge Graph/HQL、MCP/CLI/Skills、Test/Security/SRE 专项 Agent | L3 | 已发布能力与 GA/EA/Flag 混合；Worker 子能力未见统一阶段标签 | Agent 是受治理 Pipeline Step，平台提供上下文与委托控制；Oracle 仍取决于连接的 Test/Scan/Policy |
...
| Buildkite | 可组合 CI 与 Agent 执行 | Retry、Test Engine、Skills/MCP、Agent in Pipeline | L3 | 正式文档未标 Agentic CI 整体阶段；Direct MCP Endpoint 单独 Preview | 保持开放模型和可组合 Runner 控制；当前更像 Agent 底座而非通用修复产品 |
...
复杂场景已经从单 Workflow 扩展到 DeterministicOps、Orchestrator/Worker、Side Repo 和 CentralRepoOps：确定性 Step 先收集日志/测试/发布证据，Agent 进行归因与排序，Worker 按仓库和风险独立执行，原 CI/Policy/Environment Approval 决定是否合并或发布。CI Doctor 是可部署的官方参考 Workflow；Code Scanning Agentic Autofix 则以 CodeQL 复验形成更窄的安全微闭环。两者都不支持把 GitHub Actions 整体写成通用自愈产品。
...
GitLab Duo Agent Platform 在 18.8 宣布 GA，提供 Agentic Chat、基础/自定义/外部 Agent 和多 Agent Flow。Fix CI/CD Pipeline Flow 在 18.8 GA、MR Suggestion 输出在 19.2 GA；它能读取失败日志、Exit Code、MR Diff 与仓库并交付 Suggestion/MR，但本轮公开一手材料未证明候选变更前自动完成原完整 Pipeline 复验。
...
成熟度需要分三栏看：平台总览、专项 Agent 和具体执行面不能共享一个状态标签。Worker Agents 已正式发布并面向客户可用，但当前产品文档未给所有子能力统一 GA/Preview 标签；AI Test、Scribe、Hosted MCP、Investigator Pipeline 和 Scoped Permission 仍存在 Sales、Support、EA 或 Feature Flag 条件。量化案例主要集中在 AI Test Automation，且仍为 Harness 第一方发布。
...
CircleCI Chunk 把代码、构建历史、测试结果和失败模式组合起来，诊断并修复 CI 问题。官方 Changelog 已证明候选会推到分支并触发 Validation Pipeline，失败后继续尝试；受保护分支创建 Draft PR，验证失败则关闭。代码路径可判 SH3，但 Chunk 仍为 Beta，且 Validation Pipeline 是否覆盖全部 Required Checks 取决于客户配置。

Buildkite 则保持可组合路线：Retry、Test Engine、Skills/MCP、Model Provider 和 Pipeline Step 为 Agent 提供执行恢复、测试状态和构建上下文。官方参考 PR Fixer 证明客户可组合出验证循环，但本轮公开一手材料未证明平台原生通用 Patch/PR/复验闭环；其优势是开放和 Runner 控制，代价是企业需要自己组装修复状态机与接受门禁。
...
Nx 从另一个方向把自治压缩到 Task：Project Graph 和 Task Metadata 提供上下文，原失败 Task 负责复验，Eligible/Never-fix Pattern、Protected Branch 和 `SELF_HEALING.md` 约束修改范围，Auto-apply 只对选定 PR Task 开放。这比“给 Agent 整仓写权”更适合企业复制；官方称多个计划可用，但当前页面未给整套能力统一 GA 标签。参考 [[00_sources/briefs/2026-nx-self-healing-ci|Nx Self-Healing CI]]。
