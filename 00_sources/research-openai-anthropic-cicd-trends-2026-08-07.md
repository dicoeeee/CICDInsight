...
| A5 | auto mode 默认 20+ 条 block rules：禁止强推覆盖历史、批量删云存储、外发内部数据、禁用日志、装 SSH key/定时任务持久化、跨信任边界动作、直接推 main、绕过 pre-check 的部署命令等；连续 3 次/累计 20 次拒绝则升级给人工；headless 模式（`claude -p`）无 UI 可询问人类时，连续拒绝后选择终止进程 | 同上 | 2026-03-25 | GA | 2026-08-07 | 高 |
