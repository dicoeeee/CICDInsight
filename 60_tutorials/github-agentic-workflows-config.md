...
```bash
gh aw compile ci-health --strict
```

会在同目录生成 `.github/workflows/ci-health.lock.yml`。真实文件还会展开完整的运行时安装、Firewall、MCP Gateway、Artifact、成本统计和错误处理步骤，通常远长于源 Markdown。下面保留与理解控制边界最相关的结构：

> [!warning] 这是生成文件的结构节选
> 示例依据当前 `gh-aw` 生成结构整理，使用 `<generated>` 和省略注释隐藏版本相关细节，不可代替本机编译产物。`schema_version`、Action SHA、Container Digest、Cron、Job 和环境变量都可能随编译器版本与 Frontmatter 改变；不要复制后手工维护。

```yaml
# gh-aw-metadata: {"schema_version":"v4","frontmatter_hash":"<generated>","body_hash":"<generated>","strict":true,"agent_id":"copilot","engine_versions":{"copilot":"<generated>"}}
# gh-aw-manifest: {"version":1,"secrets":["GITHUB_TOKEN"],"actions":["<SHA-pinned actions>"],"containers":["<digest-pinned images>"]}
# This file was automatically generated by gh-aw. DO NOT EDIT.

name: "Weekly CI Health Review"
on:
  schedule:
    - cron: "<compiler-generated UTC cron>" # 来自 weekly on monday
  workflow_dispatch:
    inputs:
      aw_context:
        default: ""
...
# 不给整个 Workflow 默认写权限；权限下沉到各 Job。
permissions: {}
...
jobs:
  activation:
    ...
    permissions:
      ...
      contents: read
    steps:
      - name: Set up and validate the agentic runtime
        uses: github/gh-aw-actions/setup@<pinned-sha>
      # 校验 Lock、预算和凭据，并装配本次运行上下文……
  ...
  agent:
    ...
    permissions:
      ...
      copilot-requests: write
    steps:
      - name: Set up the agent, tools and network policy
        uses: github/gh-aw-actions/setup@<pinned-sha>
      - name: Load ci-health.md instructions and run the agent
        run: <compiler-generated runtime command>
      - name: Upload structured agent output
        uses: actions/upload-artifact@<pinned-sha>

  # 启用 Threat Detection 时，编译器会在这里插入独立 detection Job。
  ...
  safe_outputs:
    ...
    permissions:
      ...
      issues: write
    steps:
      - name: Download structured agent output
        uses: actions/download-artifact@<pinned-sha>
      - name: Process safe outputs
        run: <compiler-generated safe-output handler>
        # 源文件 staged: true，因此只生成预览，不实际创建 Issue。
```

这段生成物体现了源文件到 Actions 控制面的五个关键映射：

| 源文件声明 | `.lock.yml` 中的结果 | 应如何解读 |
|---|---|---|
| `on: weekly on monday` | 标准 `schedule.cron`，并加入受控的手动调用入口 | Agent 没有自建调度器，仍由 Actions Event 唤起。 |
| `permissions` | 全局 `permissions: {}` + 各 Job 的最小权限 | `agent` 只有读取和推理权限，写权限不向 Agent Job 泄漏。 |
| `tools.github` 与 `network` | Agent Runtime、MCP/Tool 配置和网络策略 | Tool 可见性被编译进运行环境，但能否成功调用仍受 Token 权限约束。 |
| Markdown 正文 | Agent Job 运行时加载 `ci-health.md` | Task 保持为可读 Markdown；修改 Frontmatter 必须重编译，正文按当前机制可运行时加载。 |
| `safe-outputs.create-issue` + `staged: true` | 独立 `safe_outputs` Job、结构化 Artifact 和预览处理器 | Agent 只能提出创建 Issue 的候选请求；当前配置不会真正写入 GitHub。 |

> [!important] 评审 Lock File 时看什么
> 不评审编译器生成的每一行实现细节，而是检查 metadata/hash、触发器、Job Graph、Job 级权限、Action/Container Pin、Tool/MCP、网络和 Safe Output 边界是否与 `.md` 的声明一致。提交时仍应同时提交 `.md` 与 `.lock.yml`。
...
- [Workflow Structure 与 Lock File Header](https://github.github.com/gh-aw/reference/workflow-structure/)
- [Compilation Process](https://github.github.com/gh-aw/reference/compilation-process/)
- [官方仓库中的真实 `.lock.yml` 示例](https://github.com/github/gh-aw/blob/main/.github/workflows/ci-coach.lock.yml)
